...
export class WeatherService {
  ...
  getTimeInfo(city: string, country: string): string {
    const currentDate = new Date();
    formatDate(currentDate, 'full', 'en-US', `${city}, ${country}`);

    // Obtener el día de la semana y la hora actual en formato localizado
    this.dayOfWeek = new Intl.DateTimeFormat('en-US', { weekday: 'long' }).format(currentDate);
    this.timeOfDay = new Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }).format(currentDate);
    const timeFormat = this.dayOfWeek + ", " + this.timeOfDay 
    return timeFormat
  }


  private getExactTime(timestamp: number, city: string, country: string): string {
    if (city != null && country != null) {
      return this.getTimeInfo(city, country);
    } else {
      return 'Unknown time';
    }
  }
  ...
  formatWeatherData(data: any): any {
    ...
    const exactTime = this.getExactTime(data?.dt, data?.name, countryName);
